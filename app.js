...
// ── STATE ──────────────────────────────────────────────────────────────────
let b64=null, itype='image/jpeg', pickerOpen=false, viewMode='list', sortBy='date';
...
$('camCloseBtn').addEventListener('click',()=>{ showView('vLibrary'); renderLib(); });
$('openCamBtn').addEventListener('click',()=>showView('vCamera'));
...
async function analyze(){
  ...
  if(!b64){ showToast('Сначала загрузите фото'); return; }
  const apiKey=getKey();
  if(!apiKey){ showToast('Добавьте API-ключ в Настройках'); showView('vSettings'); renderSettings(); return; }
  if(!navigator.onLine){ showToast('Нет интернета'); return; }

  $('resultArea').innerHTML='<div class="result-overlay show"><div class="loading-box"><div class="spin"></div><p>AI ищет объекты...</p></div></div>';
  clearOverlay();

  const system=`You are a precise visual object detector and counter. Respond ONLY with valid JSON, no markdown:
{"count":<integer>,"object":"<in Russian>","confidence":"<high|medium|low>","notes":"<1 sentence in Russian>","boxes":[{"x":<0-1>,"y":<0-1>,"w":<0-1>,"h":<0-1>}]}
boxes: one per detected object, normalized coords 0-1.`;
  ...
  try{
    ...
    $('resultArea').innerHTML=`
      <div class="result-overlay show">
        ...
        ${p.boxes?.length?'<div class="ro-hl"><i class="ti ti-checkbox" aria-hidden="true"></i><span>Объекты отмечены рамками на фото</span></div>':''}
      </div>`;
    if(p.boxes?.length) requestAnimationFrame(()=>drawBoxes(p.boxes));

    const entry={name:currentObj.label+' — '+cnt+' шт.',sub:'1 страница',count:cnt,confidence:p.confidence,thumb:b64.slice(0,400),time:Date.now()};
    docs.unshift(entry);
...
function renderLib(items){
  ...
  const body=$('libBody');
  if(!data.length){
    body.innerHTML='<div style="text-align:center;padding:60px 0;color:rgba(255,255,255,.2);font-size:13px"><i class="ti ti-camera-off" style="font-size:32px;display:block;margin-bottom:10px" aria-hidden="true"></i><p>Нет фотографий</p><p style="margin-top:6px;font-size:12px;color:rgba(255,255,255,.12)">Нажмите кнопку камеры</p></div>';
    return;
  }
...
$('keySave').addEventListener('click',()=>{
  ...
  renderSettings();
  setTimeout(()=>{ keyOpen=false; $('keyBar').classList.remove('open'); $('keyChevron').style.transform=''; },800);
});
...
function showToast(msg){
  ...
  if(!t){ t=document.createElement('div'); t.id='toast'; t.style.cssText='position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:rgba(30,31,42,.95);color:#fff;padding:10px 18px;border-radius:12px;font-size:13px;z-index:100;border:1px solid rgba(255,255,255,.1);white-space:nowrap;pointer-events:none'; document.getElementById('app').appendChild(t); }
